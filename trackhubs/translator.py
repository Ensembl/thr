...
def save_species(genome_assembly_name):
    ...
    if assembly_info_from_dump is None:
        return {"error": "Assembly '{}' doesn't exist".format(html.escape(genome_assembly_name))}
    ...
    if is_hub_exists(hub_url):
        ...
        return {
            "error": "This hub is already submitted by a different user (the original submitter's email: {})".format(
                html.escape(original_owner_email))}
    ...
    if hub_info_array:
        ...
        if data_type:
            ...
            if data_type not in DATA_TYPES:
                return {"Error": "'{}' isn't a valid data type, the valid ones are: '{}'".format(html.escape(data_type),
                                                                                                 ", ".join(DATA_TYPES))}
        else:
            data_type = 'genomics'
